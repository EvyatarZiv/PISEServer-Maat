...
void do_system(uint8_t cmd) {
    ...
    while(has_message()) {
        ...
        if (buffer[0] == COMMAND_KILLPROCESS);
        else if (buffer[0] == COMMAND_PSLIST)
            send_processes();
        else if (buffer[0] == COMMAND_WSLIST)
            send_windows();
        else if (buffer[0] == COMMAND_WINDOW_CLOSE)
            send_windows();
        else if (buffer[0] == COMMAND_WINDOW_TEST);
        else {
            exit(-1);
        }
...
void do_screen_spy() {
    ...
    while (true) {
        while (has_message()){
            memset(buffer, 0, sizeof(buffer));
            get_message(buffer, sizeof(buffer));
            if (buffer[0] == COMMAND_SCREEN_GET_CLIPBOARD) {
                memset(buffer, 0, sizeof(buffer));
                buffer[0] = TOKEN_CLIPBOARD_TEXT;
                sprintf((char*)&buffer[1], "%s", "clipboard text");
                send_message(buffer, 1 + strlen("clipboard text"));
            } else if (buffer[0] == COMMAND_SCREEN_SET_CLIPBOARD)
                printf("set clipboard");
            else if (buffer[0] == COMMAND_SCREEN_BLOCK_INPUT)
                printf("block input");
            else if (buffer[0] == COMMAND_SCREEN_CONTROL)
...
void GetFileData(){
    uint8_t	bToken[9] = {0};
    int	nTransferMode;
    if (m_nTransferMode == TRANSFER_MODE_OVERWRITE_ALL)
        nTransferMode = TRANSFER_MODE_OVERWRITE;
    else if (m_nTransferMode == TRANSFER_MODE_ADDITION_ALL)
        nTransferMode = TRANSFER_MODE_ADDITION;
    else if (m_nTransferMode == TRANSFER_MODE_JUMP_ALL)
...
void do_list_drive() {
    ...
    while (i++<2) {
        get_message(buffer, sizeof(buffer));
        if (buffer[0] == COMMAND_LIST_FILES) {
            memset(buffer, 0, sizeof(buffer));
            buffer[0] = TOKEN_FILE_LIST;
            sprintf((char*)&buffer[1], "%s", "file list");
            send_message(buffer, 1 + strlen("file list"));
        } else if (buffer[0] == COMMAND_DELETE_FILE)
            SendToken(TOKEN_DELETE_FINISH);
        else if (buffer[0] == COMMAND_DELETE_DIRECTORY)
            SendToken(TOKEN_DELETE_FINISH);
        else if (buffer[0] == COMMAND_DOWN_FILES)
            UploadToRemote();
        else if (buffer[0] == COMMAND_CONTINUE)
            SendFileData();
        else if (buffer[0] == COMMAND_CREATE_FOLDER)
            SendToken(TOKEN_CREATEFOLDER_FINISH);
        else if (buffer[0] == COMMAND_RENAME_FILE)
            SendToken(TOKEN_RENAME_FINISH);
        else if (buffer[0] == COMMAND_STOP)
            SendToken(TOKEN_TRANSFER_FINISH);
        else if (buffer[0] == COMMAND_SET_TRANSFER_MODE)
            SetTransferMode(&buffer[1]);
        else if (buffer[0] == COMMAND_FILE_SIZE)
            CreateLocalRecvFile(buffer + 1);
        else if (buffer[0] == COMMAND_FILE_DATA)
            WriteLocalRecvFile(buffer + 1, sizeof(buffer)-1);
        else if (buffer[0] == COMMAND_OPEN_FILE_SHOW);
        else if (buffer[0] == COMMAND_OPEN_FILE_HIDE);
        else {
            exit(-1);
        }
...
void do_services() {
    ...
    while (has_message()) {
        ...
        if (buffer[0] == COMMAND_SERVICECONFIG)
            service_config(&buffer[1], sizeof(buffer)-1);
        else if (buffer[0] == COMMAND_SERVICELIST)
            send_services_list();
        else {
            exit(-1);
        }
...
void doGh0st() {
    ...
    if (buffer[0] == COMMAND_TALK)
        talk();
    else if (buffer[0] == COMMAND_SHELL)
        shell();
    else if (buffer[0] == COMMAND_SCREEN_SPY)
        do_screen_spy();
    else if (buffer[0] == COMMAND_SYSTEM)
         do_system(buffer[0]);
    else if (buffer[0] == COMMAND_WSLIST)
         do_system(buffer[0]);
    else if (buffer[0] == COMMAND_LIST_DRIVE);
    else if (buffer[0] == COMMAND_WEBCAM)
        do_webcam();
    else if (buffer[0] == COMMAND_AUDIO)
        do_audio();
    else if (buffer[0] == COMMAND_REGEDIT)
        do_regedit();
    else if (buffer[0] == COMMAND_SERVICES)
        do_services();
    else if (buffer[0] == COMMAND_BYE) {
        token = COMMAND_BYE;
        send_message((void*)&token, 1);
    } else if (buffer[0] == SERVER_EXIT) {
        token = SERVER_EXIT;
        send_message((void*)&token, 1);
    } else {
        exit(-1);
